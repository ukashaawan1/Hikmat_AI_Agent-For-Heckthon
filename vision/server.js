  ...
  clientWs.on("message", (data) => {
    try {
      ...
      if (msg.type === "setup") {
        ...
        console.log(`Setting up Vision for language: ${lang}`);
        ...
        geminiWs.on("open", () => {
          const setup = {
            setup: {
              model: normalizeModelName(GEMINI_LIVE_MODEL),
              generationConfig: {
        ...
        geminiWs.on("message", (gData) => {
          ...
          if (gMsg.setupComplete) {
            console.log("Gemini Setup Complete. Sending Welcome...");
            // Send Initial Greeting turn after setup is complete
            sendJson(geminiWs, {
              clientContent: {
                turns: [{
                  role: "user",
      ...
      // 2. Handle Greet (Client might still send it, we ignore it or use it)
      if (msg.type === "greet") return; 

      // 3. Handle Media/Audio (If geminiWs is ready)
...
server.listen(PORT, () => {
  console.log(`Vision Server running on http://localhost:${PORT}`);
});
